...
Searches web for labor time estimates using multiple automotive sources
"""
...
class LaborTimeSearchEngine:
    ...
    def __init__(self):
        ...
        # Automotive forums and professional sites
        self.professional_sources = [
            "iatn.net",  # International Automotive Technicians Network
            "mitchell1.com",
            "alldata.com",
            "identifix.com"
        ]
    ...
    def validate_labor_time_results(self, results: List[Dict], repair_task: str, vin_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for result in results:
            ...
            elif any(source in url.lower() for source in self.professional_sources):
                confidence += 30  # Professional automotive source
            ...
            extracted_time = self._extract_time_estimates(content, title)
    ...
    def _extract_time_estimates(self, content: str, title: str) -> Dict[str, Any]:
    ...
    def _classify_source(self, url: str) -> str:
        ...
        elif any(source in url_lower for source in self.professional_sources):
            return "professional_automotive"
...
def labor_time_web_search(agent, repair_task: str, vehicle_info: str = "") -> Dict[str, Any]:
    ...
    try:
        # Extract VIN context
        vin_context = labor_search_engine.extract_vin_context(agent)
        ...
        for i, query in enumerate(search_queries):
            ...
            domains = labor_search_engine.labor_sources + labor_search_engine.professional_sources
            
            search_result = _execute_labor_search(query, domains)
            ...
            if search_result.get("success"):
                ...
                logger.warning(f"⚠️ Query {i+1} failed: {search_result.get('error', 'Unknown error')}")
...
def _execute_labor_search(query: str, domains: List[str]) -> Dict[str, Any]:
    """Execute labor time search with Tavily API"""
    try:
        ...
        payload = {
            ...
            "max_results": 4,  # Fewer results per query since we're doing multiple queries
        ...
        logger.error(f"Labor search API error: {e}")
...
logger.info("✅ Labor Time Web Search Tool - Phase 2 loaded successfully")
