...
export function MechanicReview({ 
  ...
  console.log('🔍 MechanicReview received diagnosticSession:', diagnosticSession);
  ...
  const getGuidanceLevelInfo = (level: 'generic' | 'basic' | 'vin' | undefined) => {
    switch (level) {
      case 'generic':
      ...
      default:
        return { icon: 'help-circle', label: 'Standard Guidance', color: '#6b7280' }
    }
  }
  ...
  const guidanceInfo = getGuidanceLevelInfo((diagnosticSession as any).guidanceLevel)
      ...
      <View style={styles.header}>
        ...
        <View style={styles.sessionInfo}>
          ...
          <Text style={styles.timestamp}>
            {(() => {
              try {
                const date = new Date(diagnosticSession.createdAt);
                return `${date.toLocaleDateString()} at ${date.toLocaleTimeString()}`;
              } catch (error) {
                return diagnosticSession.createdAt || 'Date unavailable';
              }
            })()}
          </Text>
        </View>
